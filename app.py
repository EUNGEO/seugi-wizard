...
# 페이지 환경 설정
st.set_page_config(page_title="선생님 전용 생기부 마법사 v3.3", layout="wide")
...
st.title("🛡️ 스마트 생기부 마법사 v3.3 (자유 수정 마스터판)")
st.info("💡 [선생님 맞춤형 패치] 역량명과 성취 수준(우수/보통) 문구를 공란으로 비워두어, 선생님께서 사후에 자유롭게 편집하실 수 있도록 최적화했습니다.")

# --- 🌟 안전 세션 금고 시스템 ---
if "records_db" not in st.session_state:
    st.session_state["records_db"] = []
if "temp_text_output" not in st.session_state:
    st.session_state["temp_text_output"] = ""
if "saved_success_trigger" not in st.session_state:
    st.session_state["saved_success_trigger"] = False
...
# --- 🔥 활동별 성취기준 가이드라인 (선생님 요청으로 역량명, 우수, 보통 초기화) ---
ACTIVITY_MASTER_DB = {
    "한국지리": {
        "지역공공정책서": {
            "성취기준": [
